...
export class EsPreadmission {
  ...
  toProfile() {
    const personal = {
      username: this.preStudentUsername,
      name: this.preName,
      regNo: this.admissionId,
      class: this.preClass,
      gender: this.preGender,
      section: this.preSec,
      dob: toSimpleShortDate(this.preDateofbirth),
      avatar: this.preImage,
      bloodGroup: this.preBloodGroup,
      address: this.preAddress,
    };

    const parent = {
      father: this.preFathername,
      mother: this.preMothername,
      contact: this.preContactno,
      occupation: this.occupation1,
    };
